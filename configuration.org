...
** Set repo using package and initialize use-package
   ...
   - In this setting we are installing ~solarized-theme~. This can be changed to any theme that you want. Also make sure to change ~load-theme~ setting.
...
** Change multiple files in minibuffer using wgrep
   - Can use ~C-c g~ (~helm-git-do-grep~) command in a git project to search files using grep
   - Can use ~C-c C-s~ to save the buffer
