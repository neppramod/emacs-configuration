...
If you press F6 key in your keyboard, it should switch between doom-monokai-spectrum and doom-one-light themes. If you want to go back to the default theme press ~Ctrl + C and F6~.
...
** General Key
 ...
 ; Use following example, when needed
 ;  (my/general-keys "m" '(hydra-move/body :which-key "move keys"))
   #+end_src
...
("c" dap-java-run-test-class "run test class")
("i" delete-other-windows "maximize")
("q" nil "quit")
)

;(key-chord-define-global "RR" 'hydra-run/body)
(my/general-keys "r" '(hydra-run/body :which-key "run java"))
#+end_src
